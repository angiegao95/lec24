...
def init_football(csv_file_name=BB_FILE_NAME):
    global fb_seasons
    with open(csv_file_name) as f:
        reader = csv.reader(f)
        next(reader) # throw away headers
...
def get_bball_seasons(bb_seasons, sortby='year', sortorder='desc'):
...
def get_football_seasons(bb_seasons, sortby='year', sortorder='desc'):
